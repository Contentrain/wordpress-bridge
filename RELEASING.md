...
Publishing is a public act and the founder's decision: a person pushes the tag,
nothing else does. After the version bump has merged to `main`:

```bash
git tag -a v0.3.0 -m "Contentrain Bridge 0.3.0" && git push origin v0.3.0
```

`.github/workflows/release.yml` then runs every gate in `ci.yml` in the same run,
checks that the tag equals the plugin version and that `readme.txt` has its
changelog entry, and attaches the archive **that run built and tested**
(`contentrain-bridge.zip` and its `.sha256`) to a GitHub Release, with the
changelog entry as notes. Nothing is rebuilt after the gates.

Users install from `https://github.com/Contentrain/wordpress-bridge/releases/latest`
(Plugins > Add New > Upload Plugin). Updates are manual until the plugin is on
WordPress.org: the new ZIP is uploaded over the installed one. There is
deliberately no self-hosted update check: WordPress.org forbids one, and it
would have to be removed at submission.
...
| **B-04 SEO/redirect runtime coverage** | Yoast SEO and Redirection are tested at runtime: `npm run test:seo` compares every exported head value and redirect with what the live site serves. **Since 0.3.0 (BR-22):** Rank Math 1.0.279, AIOSEO 5.0.2 and SEOPress 10.2 run live too, one at a time, and Bridge's rendered title, description, robots and canonical equal the head each serves on every fixture page (`tests/seo-live.php`). Still stored-format only: their redirects, Yoast Premium redirects and Safe Redirect Manager (real table/meta/option shapes, seeded) — their live redirects are not tested. Multilingual SEO (WPML/Polylang) is not covered. `@contentrain/verify@0.2.0` cannot yet report a page the migration dropped (`status.baseline-page-missing` is unreleased) |
...
| **B-11 real consumption** | **Narrowed.** `npm run test:e2e` runs in CI: the richest fixture (ACF, Yoast, Redirection, menu, media) is exported, built by Astro through `@contentrain/query`'s loader with one page per entry and every relation, media file, menu item and redirect checked; three WordPress edits then reach Git as exactly those three entries and the right pages after a rebuild. The GitHub leg runs in CI against a dedicated repository (`BRIDGE_TEST_REPO`/`BRIDGE_TEST_TOKEN` secrets): each run cuts its own `e2e/<run>` base branch from the repository's first commit, delivers and accepts against it, and deletes every branch it made on exit, so no run inherits another's content and nothing is reset; the run fails if any part of the token reaches its output. Without the secrets it reports `SKIP` and the local leg commits the same trees to a real Git repository. Studio reading and editing the delivered store is waiting on staging and has not been run |
| **B-01 directory review** | **Narrowed, 2026-09-18.** Manual review done: no trialware/upsell language anywhere in the plugin; the one outbound request (`wp_remote_request` to `api.github.com`) is disclosed in `readme.txt` and gated behind an explicit consent checkbox, nonce and capability check; the `migrate.contentrain.io` link is a plain anchor the admin must click, not a request; zero third-party code ships in the archive — the three npm packages are dev/test-only (excluded by `tools/package.py`) and all MIT, compatible with `GPL-2.0-or-later`. Still missing: the WP.org submission itself, and CI still only lints PHP 7.4/8.3 syntax and runs acceptance on one WordPress/ACF version; multisite behaviour is untested. **Large sites since 0.3.0 (BR-24):** the `perf` workflow exports 5,000 posts, 20,000 media records and 20,000 comments the way Migrate does, at 128M and 64M, and fails past 30 minutes (measured: about two minutes) |
| **Source coverage** | `complete_source_coverage: false` is correct and must not be presented otherwise. **Narrowed, 2026-09-18 (B-03):** `relationship`, `post_object`, `taxonomy`, `gallery`, `user` and `link` now become real relations (`Acf::reference()`), and `flexible_content` becomes a collection with its layout name preserved (`Acf::flexible()`), each measured against a real ACF 6.8.10 fixture with zero silent drops. **Narrowed, 2026-09-18 (B-05):** menus resolve their target to kind/post_type/taxonomy/slug with an unresolved target or parent reported rather than guessed at (`Models::menus()`), and language pairs (`bridge/language-pairs.json`) are one row per translation group against a real Polylang fixture, not one per post. **Source universe (B-02), 2026-09-18:** every export writes `bridge/coverage.json` — each content-bearing source (every post type and status, taxonomies, term meta, post and attachment meta, comments, users, options, widgets, Customizer, sticky, page templates, shortcodes, embeds, reusable-block references, registered patterns, network sites, and every database table) with a database count split into exported / excluded:reason / unsupported:reason; `npm run test:coverage` requires every source to add up in both scopes and Bridge's RawIR to equal a WXR export read by `@contentrain/wp-import` on 12 post fields, attachments, terms, authors and comments. `complete_source_coverage` stays false because the report itself names what is unsupported: shortcode and embed rendering, unregistered post types, and plugin tables it does not read. **Interface text (B-08), 2026-09-18:** widget, menu-label and Customizer (theme_mods) text and gettext/HTML/echo/JS scanning are measured by `npm run test:text` against a fixture theme and five rendered pages: every occurrence is accounted for (transfer / exclude with reason / error), deduplication only on identical text+locale+context. **Partial translation, 2026-09-18:** a missing locale never gets a silent copy of another locale's text; `bridge/language-pairs.json` names the gap (`missing_translations`) instead. For a `document`-kind model (a real WordPress `post`) this is already `contentrain validate`'s own `Missing translation` **warning**, not an error — verified against the real installed validator, not assumed. Taxonomy terms and menu items are `i18n: false` since B-05, so an untranslated term or an untranslated menu target was never a parity question to begin with. **Full field-type coverage (B-03), 2026-09-18:** `clone` (seamless and group display) and ACF Options Page were measured against Secure Custom Fields (SCF, WP.org, the same ACF fork/database format, its Pro-equivalent field types free) since ACF Pro itself has no free license to test against. A seamless clone flattens into its parent under the source fields' own names, exactly as if they had been added directly; a group-display clone becomes a relation to its own model, shaped like the source field group, the same way a native `group` field does. An Options Page becomes a `singleton` model with a guaranteed `page_title` title field regardless of what its configured fields are named. Multiple Options Pages commonly share the same default storage `post_id` ('options') unless a site gives one its own; fields are attributed by the field groups whose own `options_page` location names that specific page (`Source::acf_fields_for_options_page()`), not by everything stored at that shared `post_id` — an earlier version of this leaked every page's fields into every other page's model, found in review and fixed. A post outside every ACF field group's own location rule carries no `acf` key on its raw record at all — an earlier version of this always set one (`"acf": []` when empty), changing the inventory fingerprint of every non-ACF record on the first export after upgrading even though nothing about those records changed; found in review, fixed, and covered by a check that such a record's raw shape is unchanged. All 9 target field types now have real, empirical export coverage; **ACF Pro's own runtime is still unverified** — SCF is the same fork family and on-disk format, not a substitute for the licensed product itself. Testing under SCF also surfaced one genuine ACF/SCF-family bug, not assumed: a `flexible_content` field whose layouts share a sub-field name (`heading`) returns that field's raw, unformatted row value keyed by field *key* rather than name whenever two layouts use different keys for it — fixed in `Acf::flexible()` with the same dual name-or-key row lookup `shape()` already used for group/repeater rows. (A second suspected bug — a top-level `user` field sharing its exact name with an unrelated nested repeater sub-field misresolving as a scalar — turned out to be this plugin's own test fixture calling `update_field()` by an ambiguous *name* rather than the field's *key*; a real edit through wp-admin, and this plugin's own export code, never does that. Fixed by saving the fixture by key; nothing in the plugin itself needed changing.) **Options Page storage is one `options_{name}` row plus one `_options_{name}` reference row per field** (or `{post_id}_{name}` for a page with its own `post_id`), not one row for the whole page, and this plugin's own export declares it `i18n: false` since none of that is per-language. A third-party plugin can still add a translated copy this plugin does not read — WPML + ACFML (see [WPML's ACF Options docs](https://wpml.org/documentation/related-projects/translate-sites-built-with-acf/), no free tier to verify against) or the free WP.org plugin [ACF Options for Polylang](https://wordpress.org/plugins/acf-options-for-polylang/), measured against the real plugin 2.0.0, installed for real. It suffixes a Polylang **locale** (`da_DK`, not the language slug `da`) onto *every* `post_id`, including the default `options` one — and, because it never configures ACF's own `default_language` setting, even the *default* language gets a redundant suffixed copy once an admin's language filter has touched it; an earlier version of this only matched a non-default `post_id` and a hand-planted key rather than the plugin's own real rows, found in review and fixed (`Coverage::translation_suffixes()` now matches both the `locale` and `slug` forms of every configured language, default included). Reading now runs inside the plugin's own untranslated context (`bea_aofp_switch_to_untranslated()` / `restore_current_lang()`), so the exported default-locale value no longer depends on whatever language an admin's admin-bar language filter happens to be set to when they run the export — also found in review (measured: exporting with `curlang=da` silently wrote the Danish value as the default) and fixed, with a test that exports under `curlang=da` and asserts the untranslated value still comes back. Every such row (value + reference, for any configured locale including the default) is reported `unsupported:options-page-translation` in `bridge/coverage.json` rather than folded into `excluded:configuration-not-content`, and a repeater/group/flexible_content/group-display-clone field's own sub-rows are now attributed to that field (`Coverage::options_field_for()`) instead of falling through the same way. A `contentrain-bridge` warning names the plugin when it (or WPML + ACFML, unverified) is active. **Still open: Options Page translations are not migrated**, only the default-language values are — closing that gap means reading each detected plugin's own per-language storage, not attempted here. WPML's own multilingual post/term code paths (unrelated to Options Pages) have no free tier to install and test against, so they remain structurally present but unverified; a `collection`-kind model (a WordPress `page` or any other post type) with a genuinely untranslated entry still fails `contentrain validate`'s `Entry parity` **error** — the published validator's own design (`config.locales.supported` is one project-wide list with no per-model or per-entry override; see `reports/BS-4-contract.md` for the exact source and the upstream change this needs), not something this plugin's own code can close; a real third-party theme or page builder, block-theme template parts beyond HTML text nodes, and strings a plugin renders only for logged-in users remain unmeasured. **Taxonomy term references under an admin's language filter, 2026-09-18:** a post's own category/tag, and a raw `term_taxonomy` row enumerated directly, are re-read by slug or id to build the term's model and the inventory fingerprint; Polylang filters `get_term_by()`/`get_terms()` to the admin's current language filter (`curlang`) whenever a query does not explicitly disable it, so a term tagged with any language other than curlang used to read back as unresolvable and stop the export with a `RuntimeException` — found in review (BO-10) against the real installed plugin, independent of any specific field type, and fixed (`Source::term_by()`, `lang => ''`, Polylang's own documented way to disable the filter). Covered by a full export run under `curlang=da` against an explicitly `en`-tagged fixture term, plus a direct check that the same term resolves to the same term id whether curlang is unset, Danish or English. The same admin's language filter also leaked into the inventory's site-wide comment count (BO-12): `wp_count_comments()` falls through to a comment query Polylang filters by `curlang` exactly like it filters terms, so `bridge/inventory.json`'s `comments` totals silently described one language's comments rather than the whole site's — fixed (`Source::without_curlang()` clears and restores `curlang` around the call), covered by a check that the count is unchanged under `curlang=da`. |
