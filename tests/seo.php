...
check( 'conditional-match:htaccess' === $ht['htcond']['reason'] && array( 'RewriteCond %{HTTP_HOST} ^never\\.example$' ) === $ht['htcond']['condition'] && 404 === fetch( '/ht-cond-' . $run )['status'], 'a RewriteRule behind a RewriteCond is excluded with its condition, and the site does not serve it here' );
