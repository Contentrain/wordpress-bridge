...
if ( is_dir( $root ) || get_option( 'contentrain_bridge_revision' ) || false !== get_option( 'contentrain_bridge_changes' ) || false !== get_option( 'contentrain_bridge_key' ) || false !== get_option( 'contentrain_bridge_key_seen' ) || false !== get_option( 'contentrain_bridge_key_exports' ) || get_user_meta( $admin->ID, 'contentrain_bridge_job_' . get_current_blog_id(), true ) || wp_next_scheduled( 'contentrain_bridge_cleanup' ) ) {
	throw new RuntimeException( 'Uninstall left private snapshots or bookkeeping behind.' );
}
echo "PASS: WordPress uninstall removes snapshots, ownership, revision, change log, connection key and cron.\n";
