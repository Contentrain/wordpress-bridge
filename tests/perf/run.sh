...
memory="${MEMORY:-128M}" limit="${LIMIT:-1800}"
finish() {
  status=$?
  if [ "$status" != 0 ]; then
...
"${compose[@]}" exec -T wordpress sh -c "echo 'memory_limit=$memory' > /usr/local/etc/php/conf.d/zz-bridge-perf.ini && apache2ctl -k graceful" >/dev/null 2>&1
