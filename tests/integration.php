...
if ( $has_acf ) {
	...
	check( array( 'type' => 'array', 'items' => array( 'type' => 'object', 'fields' => array( 'person' => array( 'type' => 'string' ), 'quote' => array( 'type' => 'text' ) ) ) ) === $page_fields['acf_quotes'], 'a repeater is an array of objects with its sub-fields\' own types' );
	check( array( array( 'person' => 'Ada', 'quote' => 'It reads like my site.' ), array( 'person' => 'Grace', 'quote' => 'The diff is the content.' ) ) === $page_entry['acf_quotes'], 'every repeater row is kept, in order, with its own values' );
	check( array( 'type' => 'object', 'fields' => array( 'heading' => array( 'type' => 'string' ), 'cta' => array( 'type' => 'string' ) ) ) === $page_fields['acf_hero'], 'a group is an object' );
	check( array( 'cta' => 'Start now', 'heading' => 'Own your words' ) == $page_entry['acf_hero'], 'the group\'s values are in place' );
	...
	check( '1 Example Street' === $page_entry['acf_office']['address'] && 52.37 === $page_entry['acf_office']['lat'] && 14 === $page_entry['acf_office']['zoom'], 'a map keeps its address and position' );
	check( array( 'type' => 'array', 'items' => array( 'type' => 'select', 'options' => array( 'news', 'guides', 'events' ) ) ) === $page_fields['acf_topics'] && array( 'guides', 'news' ) === $page_entry['acf_topics'], 'a checkbox is an array of its choices' );
	check( 'array' === $page_fields['acf_audience']['type'] && array( 'ed' ) === $page_entry['acf_audience'], 'a multi-select is an array of its choices' );
	check( array( 'type' => 'select', 'options' => array( 'gold', 'silver' ) ) === $page_fields['acf_tier'] && ! isset( $page_entry['acf_tier'] ), 'a stored choice the field no longer offers is left out, not a failed export' );
	...
	check( array( array( 'question' => 'Where do I start?', 'links' => array( array( 'link' => array( 'title' => 'Start here', 'url' => 'https://example.test/start' ) ) ) ), array( 'question' => 'Is it mine?' ) ) == $page_entry['acf_faq'], 'nested rows keep their values; an empty nested list and an empty target are left out' );
	...
	check( array( 'target' => '_blank', 'title' => 'Read more', 'url' => 'https://example.test/read-more' ) == $page_entry['acf_cta_link'], 'a link keeps its label and target alongside the URL' );
	...
	check( array( 'layout', 'heading', 'body', 'quote' ) === array_keys( $sections_field['items']['fields'] ), 'a flexible row\'s fields are the union of its layouts\' fields' );
